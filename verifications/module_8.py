...
def basic_line_follower(robot, image, td):
    ...
    if not td:
        td = {
            ...
            "end_time": time.time() + 30,  # Extended time for multiple checkpoints
            "data": {
                "reached_checkpoints": [False] * len(basic_line_follower_checkpoints),
    ...
    for i, (y, x) in enumerate(basic_line_follower_checkpoints):
    ...
    if robot and robot.position_px:
        ...
        for i, (y, x) in enumerate(basic_line_follower_checkpoints):
            if not td["data"]["reached_checkpoints"][i] and np.linalg.norm([robot_x - x, robot_y - y]) < 50:
                ...
                text = f"Checkpoint {i+1}/{len(basic_line_follower_checkpoints)} reached!"
...
def pid(robot, image, td):
    ...
    if not td:
        td = {
            ...
            "end_time": time.time() + 30,  # Extended time for multiple checkpoints
            "data": {
                "reached_checkpoints": [False] * len(pi_checkpoints),
                "task_completed": False
            }
        }
    ...
    # Place checkpoint markers on all uncompleted checkpoints
    for i, (y, x) in enumerate(pi_checkpoints):
        if not td["data"]["reached_checkpoints"][i]:
            # Make sure we stay within image bounds
            y_start = max(0, y - 30)
            y_end = min(image.shape[0], y + 30)
    ...
    if robot and robot.position_px:
        ...
        for i, (y, x) in enumerate(pi_checkpoints):
            if not td["data"]["reached_checkpoints"][i] and np.linalg.norm([robot_x - x, robot_y - y]) < 50:
                ...
                text = f"Checkpoint {i+1}/{len(pi_checkpoints)} reached!"
...
def pid(robot, image, td):
    ...
    if not td:
        td = {
            ...
            "data": {
                "reached_checkpoints": [False] * len(pid_checkpoints),
                "task_completed": False
            }
        }
    ...
    # Place checkpoint markers on all uncompleted checkpoints
    for i, (y, x) in enumerate(pid_checkpoints):
        if not td["data"]["reached_checkpoints"][i]:
            # Make sure we stay within image bounds
            y_start = max(0, y - 30)
            y_end = min(image.shape[0], y + 30)
    ...
    if robot and robot.position_px:
        ...
        for i, (y, x) in enumerate(pid_checkpoints):
            if not td["data"]["reached_checkpoints"][i] and np.linalg.norm([robot_x - x, robot_y - y]) < 50:
                ...
                text = f"Checkpoint {i+1}/{len(pid_checkpoints)} reached!"
